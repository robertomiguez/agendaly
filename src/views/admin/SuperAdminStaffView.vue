...
const filteredStaff = computed(() => {
  if (!searchQuery.value) return adminStore.staff
  const q = searchQuery.value.toLowerCase()
  return adminStore.staff.filter(s => 
    s.name.toLowerCase().includes(q) || 
    s.providers?.business_name?.toLowerCase().includes(q)
  )
})

async function openDeactivate(staffId: string) {
...
async function handleConfirm() {
  ...
  try {
    await adminStore.toggleStaffActive(selectedStaff.value, false, deactivationReason.value)
...
async function handleActivate(staffId: string) {
  try {
    await adminStore.toggleStaffActive(staffId, true)
  } catch (e) {
    console.error(e)
  }
}
...
<template>
  <div class="space-y-6">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <h1 class="text-2xl font-bold text-slate-900">Platform Staff</h1>
    ...
    <div v-if="adminStore.loading" class="flex flex-col items-center justify-center py-24 bg-white rounded-2xl border border-slate-200 shadow-sm">
      ...
      <p class="text-slate-500 font-medium">Fetching platform staff...</p>
    </div>
    ...
    <div v-else class="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
      ...
      <div v-if="filteredStaff.length > 0" class="hidden md:block overflow-x-auto">
        <table class="w-full text-left">
          ...
          <tbody class="divide-y divide-slate-100">
            <tr v-for="member in filteredStaff" :key="member.id" class="hover:bg-slate-50 transition-colors">
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="h-10 w-10 rounded-full bg-slate-100 flex items-center justify-center mr-3 overflow-hidden">
                  ...
                  <div>
                    ...
                    <div class="text-xs text-slate-400">{{ member.email || 'No email' }}</div>
                  </div>
                </div>
              </td>
              <td class="px-6 py-4">
                <div class="flex items-center text-sm text-slate-600">
                  ...
                  {{ member.providers?.business_name || 'Individual' }}
                </div>
              </td>
              <td class="px-6 py-4">
                 <span class="text-sm text-slate-600 capitalize">{{ member.role || 'Staff' }}</span>
              </td>
              <td class="px-6 py-4">
                <span 
                  :class="[
      ...
      <div v-else class="py-24 text-center">
        <div class="inline-flex items-center justify-center h-16 w-16 rounded-full bg-slate-50 text-slate-300 mb-4">
          <UserPlus2 class="h-8 w-8" />
        </div>
        <h3 class="text-lg font-bold text-slate-900">No staff members found</h3>
    ...
    <div v-if="showModal" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
       <div class="bg-white rounded-2xl max-w-sm w-full p-6 shadow-xl">
          <h3 class="text-lg font-bold text-slate-900 mb-2">Deactivate Staff</h3>
